...
- Tài khoản Quản lý gọi cùng endpoint → thành công (200).
...
# ── Quản lý gọi → 200 + danh sách (không mật khẩu) ──────────────────────────
def test_quan_ly_goi_users_200(client, dao_voi_quan_ly):
    ...
    for d in danh_sach:
        ...
        assert set(d.keys()) == {"ma_user", "ten_user", "tendangnhap", "sdt",
                                 "ma_nhom_quyen", "ten_nhom_quyen", "trang_thai"}


def test_quan_ly_goi_users_thay_du_vai_tro(client, dao_voi_quan_ly):
    _dang_nhap(client, "quanly1")
...
def test_tai_khoan_ban_goi_users_403(client, dao_voi_quan_ly):
    # Giả lập Quản lý đã bị khóa (phiên vẫn còn tồn tại từ trước khi khóa)
    dao_voi_quan_ly.thong_tin[2]["trang_thai"] = "banned"
